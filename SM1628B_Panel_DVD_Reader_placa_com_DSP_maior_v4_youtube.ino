...
/**********************************************************/
/*ATT:The model SM1628B is not equal to TM1628, the first */
/*use 16bits memory map and the second use 12bits to fill */
/*memory map.                                             */
/**********************************************************/
...
#define SM1628B_in 7// If 0 write LCD, if 1 read of LCD
#define SM1628B_clk 8 // if 0 is a command, if 1 is a data0
#define SM1628B_stb 9 // Must be pulsed to LCD fetch data of bus
...
unsigned int wheels[]={'b', 'c', 'e', 'd', 'g', 'f'}; // Att: The sequence is already like the wheels is contructed!
...
//0         (7654321) // Position of GRIDs
         ...
         (0b0000000000000000), // "e" of all digits  // Symbol Play
         (0b0000000000000000), // "f" of all digits  // Symbol Pause
         (0b0000000001000000)  // "g" of all digits  // Symbol Wheel
};
//
void setup() {
...
//only here I active the enable of interrupts to allow run the test of SM1628B
...
void SM1628B_init(void){
  ...
  // Configure SM1628B display (grids)
...
void cmd_without_stb(unsigned char a){
  ...
  //This don't send the strobe signal, to be used in burst data send
         for (mask = 00000001; mask>0; mask <<= 1) { //iterate through bit mask
           digitalWrite(SM1628B_clk, LOW);
                 if (data & mask){ // if bitwise AND resolves to true
                    digitalWrite(SM1628B_in, HIGH);
                 }
                 else{ //if bitwise and resolves to false
                   digitalWrite(SM1628B_in, LOW);
                 }
          delayMicroseconds(5);
          digitalWrite(SM1628B_clk, HIGH);
          delayMicroseconds(5);
         }
   //digitalWrite(SM1628B_clk, LOW);
}
...
void clear_SM1628B(void){
  /*
  Here I clean all registers 
  Could be done only on the number of grid
  to be more fast. The 12 * 3 bytes = 36 registers
  */
      for (int n=0; n < 14; n++){  // important be 10, if not, bright the half of wells./this on the SM1628B of 6 grids)
...
void updateDisplay(void){
  //Note: This sequence is to a panel with SM1628B and 6 DSP de 7segm e 1 de digito 1
...
unsigned char btn1 = 0x41;
  ...
  //This don't send the strobe signal, to be used in burst data send
         for (int z = 0; z < 4; z++){
             //for (mask=00000001; mask > 0; mask <<= 1) { //iterate through bit mask
                   for (int h =8; h > 0; h--) {
                      digitalWrite(SM1628B_clk, HIGH);  // Remember wich the read data happen when the clk go from LOW to HIGH! Reverse from write data to out.
                      delayMicroseconds(2);
              ...
              Serial.print(" - " );
                        ...
                        if (z==1){
                           if(array[4] == 1){
                             ...
                             //digitalWrite(SM1628B_onGreen, !digitalRead(SM1628B_onGreen));
                            }
                        } 
